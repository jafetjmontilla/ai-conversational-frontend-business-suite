...
export default function KnowledgePage() {
  ...
  // Carga inicial y suscripción Socket.IO (sustituye el polling de listProtocolDrafts)
  useEffect(() => {
    if (!businessId) return;
    loadDrafts();
  ...
  const handleSendNarrative = async () => {
    ...
    try {
      ...
      setNarrative("");
      // La lista se actualizará por Socket.IO cuando el worker envíe el webhook (protocolDraft:updated)
    } catch (e: unknown) {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Generar conocimiento</CardTitle>
          <CardDescription>
