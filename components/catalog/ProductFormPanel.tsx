...
function createEmptyProduct(isSellableDefault: boolean): ProductWithDetails {
  return {
    _id: "",
    name: "",
    description: "",
...
export function ProductFormPanel({
  ...
  useEffect(() => {
    if (!isCreate) return;
    setProduct(createEmptyProduct(hasCapability(installedApps, "product.sellable")));
    setPendingPhotoFiles([]);
    setLoading(false);
    setActiveTab("general");
  }, [isCreate, installedApps]);

  useEffect(() => {
    if (!businessIdDoc || isCreate) return;
    let cancelled = false;
    setLoading(true);
    fetchApiV1({
  ...
  }, [businessIdDoc, productId, isCreate]);
  ...
  useEffect(() => {
    if (!variantsDialogOpen || !businessIdDoc) return;
    fetchApiV1({
      query: queries.getAttributes,
      type: "json",
      variables: { id: businessIdDoc },
  ...
  }, [variantsDialogOpen, businessIdDoc]);

  const validateProduct = (draft: ProductWithDetails) => {
    const isSellable = draft.is_sellable !== false;
    if (!draft.name.trim()) {
    ...
    if (
      !isSellable &&
      !hasCapability(installedApps, "product.rawMaterial") &&
      !canUseOfferingsCatalogProduct(installedApps)
    ) {
      toast.error(getCapabilityHintPlain("product.rawMaterial"));
      return false;
    }
    if (isSellable && draft.hasBillOfMaterials && !(draft.requiredMaterials?.length)) {
  ...
  const handleGenerateResult = (draft: OfferingsImportDraft) => {
    ...
    setProduct((prev) => {
      ...
      if (prod.category_hint && categories.length) {
        ...
        if (match) next.category_id = match._id;
      }
      return next;
    });
    ...
    try {
      const created = (await fetchApiV1({
        ...
        variables: {
          ...
          args: {
            ...
            hasBillOfMaterials: product.is_sellable !== false ? (product.hasBillOfMaterials ?? false) : false,
            requiredMaterials:
              product.is_sellable !== false && product.hasBillOfMaterials
      ...
      if (pendingPhotoFiles.length > 0) {
        const full = (await fetchApiV1({
          query: queries.getProduct,
          type: "json",
          variables: { id: businessIdDoc, _id: created._id },
        })) as ProductWithDetails | null;
        const defaultVariant =
          full?.variants?.find((v) => v.status !== false && !v.deleted_at) ?? full?.variants?.[0];
  ...
  const handleSaveProduct = async () => {
    if (!businessIdDoc || !product || isCreate) return;
    if (!validateProduct(product)) return;
    setSaving(true);
    try {
      await fetchApiV1({
  ...
  if (isCreate && !canEdit) {
    return (
      <Card id="card-right" className={cn("flex h-full flex-col border-none", className)}>
        <CardContent className="flex flex-1 items-center justify-center p-6">
          <p className="text-center text-muted-foreground text-sm">No tienes permiso para crear productos.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const postCreateHint = "Disponible después de crear el producto.";
  ...
  return (
    <Card id="card-right" className={cn("flex h-full flex-col border-none overflow-hidden", className)}>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className={cn("min-w-0")}>
            ...
            <CardDescription className="text-xs">
              {isCreate
                ? "Completa los datos generales. Al guardar podrás configurar variantes, modificadores y disponibilidad."
                : "Edita el producto y sus opciones de catálogo."}
            </CardDescription>
          </div>
      ...
      <CardContent className="min-h-0 flex-1 overflow-y-auto pb-6">
        ...
        >
          <TabsList variant="line" className="h-auto w-full shrink-0 flex-wrap gap-x-1 gap-y-1">
            <TabsTrigger variant="line" value="general">
              General
            </TabsTrigger>
            <TabsTrigger variant="line" value="variants" disabled={isCreate}>
              Variantes
            </TabsTrigger>
            <TabsTrigger variant="line" value="modifiers" disabled={isCreate}>
              Modificadores
            </TabsTrigger>
            <TabsTrigger variant="line" value="availability" disabled={isCreate}>
              Disponibilidad
            </TabsTrigger>
          </TabsList>
          {isCreate ? (
            <p className="mt-2 text-xs text-muted-foreground">
              Las pestañas Variantes, Modificadores y Disponibilidad se habilitan al crear el producto.
            </p>
          ) : null}
          ...
          <TabsContent value="general" className="mt-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <ProductPhotosField
                ...
                productId={isCreate ? null : product._id}
                defaultVariantId={defaultVariant?._id ?? null}
              ...
              <div>
                ...
                {isCreate ? (
                  <p className="text-xs text-muted-foreground mt-1">{postCreateHint}</p>
                ) : null}
              </div>
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              {isCreate ? (
                <>
                  <Button type="button" onClick={() => void handleCreateProduct()} disabled={saving}>
                    {saving ? "Guardando…" : "Crear producto"}
                  </Button>
                  <Button type="button" variant="outline" onClick={resetCreateForm} disabled={saving}>
                    Limpiar
                  </Button>
                </>
              ) : (
                <>
                  <Button type="button" onClick={() => void handleSaveProduct()} disabled={saving}>
          ...
          <TabsContent value="variants" className="mt-4 space-y-4">
            {isCreate ? (
              <p className="text-sm text-muted-foreground">{postCreateHint}</p>
            ) : (
              <div className="rounded-lg border space-y-3 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Button onClick={() => setVariantsDialogOpen(true)} size="sm">
          ...
          <TabsContent value="modifiers" className="mt-4 space-y-4">
            {isCreate ? (
              ...
              <p className="text-sm text-muted-foreground">
                Los modificadores solo aplican a productos vendibles.
              </p>
          ...
          <TabsContent value="availability" className="mt-4 space-y-4">
            {isCreate ? (
              ...
              <p className="text-sm text-muted-foreground">
                La vista previa de disponibilidad aplica a productos vendibles con stock directo o receta.
              </p>
            )}
          </TabsContent>
        </Tabs>
        ...
        <Dialog open={variantsDialogOpen} onOpenChange={setVariantsDialogOpen}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setVariantsDialogOpen(false)}>Cerrar</Button>
