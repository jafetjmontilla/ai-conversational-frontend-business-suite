...
export function AppSuiteDetailDialog({
  ...
  const handleDetailOpenChange = (next: boolean) => {
    if (!next) setConfirmUninstallOpen(false);
    onOpenChange(next);
  };
      ...
      <Dialog open={confirmUninstallOpen} onOpenChange={setConfirmUninstallOpen}>
        <DialogContent className="max-w-md sm:rounded-2xl">
          <DialogHeader>
            ...
            <DialogTitle>¿Desinstalar «{app.title}»?</DialogTitle>
            <DialogDescription className="space-y-2 text-left">
              <span className="block">
                Se ocultará del menú y las funciones que dependen de esta app quedarán
                deshabilitadas hasta que la vuelvas a instalar.
              </span>
              <span className="block">
                Puedes reinstalarla después desde la Suite de aplicaciones.
              </span>
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            >
              {installing ? "Procesando…" : "Sí, desinstalar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
