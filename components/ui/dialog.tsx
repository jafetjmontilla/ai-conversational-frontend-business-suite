  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        // no usar stiles en linea hacerlo todo con tailwind
        style={{
          ...(heightKeyboard > 0 ? {
            maxHeight: `calc(100% - ${heightKeyboard}px)`,
            overflowY: 'auto',
          } : {
          })
        }}
        ref={ref}
        className={cn(
          `fixed ${isMounted ? 'visible' : 'invisible'} ${heightKeyboard > 0 ? `top-0 translate-y-0` : 'top-[50%] translate-y-[-50%]'} left-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] gap-4 border bg-background px-3 py-6 md:p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg`,
          className
        )}
        {...props}
      >
