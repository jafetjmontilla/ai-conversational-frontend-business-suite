...
export default function MobileNavSheet({ basePath, isAuthenticated, onLogout }: MobileNavSheetProps) {
  const items = [
    ...
    { label: 'Temas', href: `${basePath}/theme-demo` },
        ...
        <div className="px-2 pt-2 pb-4">
          {isAuthenticated ? (
            ...
            <div className="flex gap-2">
              <Button asChild variant="outline" className="flex-1"><Link href={`${basePath}/login`}>Iniciar sesión</Link></Button>
              <Button asChild className="flex-1"><Link href={`${basePath}/register`}>Registrarse</Link></Button>
            </div>
          )}
        </div>
      </SheetContent>
